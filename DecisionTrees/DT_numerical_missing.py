#Decision Trees - ID3 Algorithm - Bank Evaluation Task (Numerical Variables)
...
col_names = ['age', 'job', 'marital', 'education', 'default', 'balance', 'housing', 'loan', 'contact', 'day', 'month', 'duration', 'campaign', 'pdays', 'previous', 'poutcome', 'label']
...
#Reading datasets
train_data = pd.read_csv("Bank/train.csv", names=col_names)
test_data = pd.read_csv("Bank/test.csv", names=col_names)

# Dealing with Numerical Values - convert to binary with media as threshold
for i in train_data.columns[train_data.dtypes == 'int64']:
    train_data[i] = train_data[i].apply(lambda x: 1 if x > train_data[i].median() else 0)

for i in test_data.columns[test_data.dtypes == 'int64']:
    test_data[i] = test_data[i].apply(lambda x: 1 if x > test_data[i].median() else 0)

# Dealing with Missing values
for i in train_data.columns:
    if train_data[i].dtype!='int64':
        if np.sum([train_data[i].values == 'unknown']) > 0:
            for j in train_data[i].value_counts().keys():
                if j != 'unknown':
                    train_data[i] = train_data[i].replace({'unknown': j})
                    break

for i in test_data.columns:
    if test_data[i].dtype!='int64':
        if np.sum([test_data[i].values == 'unknown']) > 0:
            for j in test_data[i].value_counts().keys():
                if j != 'unknown':
                    test_data[i] = test_data[i].replace({'unknown': j})
                    break
...
class ID3:
    ...
    def test_data(self,test_data):  # calling predict function for each set of data in our dataset
        predictedlabels = []
        for i in range(len(test_data)):
            predictedlabels.append(self.predict(test_data.iloc[i], self.root))

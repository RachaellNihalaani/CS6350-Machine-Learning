...
def age_cat(val):
    if val>np.median(train_data['age']):
        return 'yes'
    return 'no'
train_data['age']=train_data['age'].map(age_cat)

def bal_cat(val):
    if val>np.median(train_data['balance']):
        return 'yes'
    return 'no'
train_data['balance']=train_data['balance'].map(bal_cat)

def day_cat(val):
    if val>np.median(train_data['day']):
        return 'yes'
    return 'no'
train_data['day']=train_data['day'].map(day_cat)

def dur_cat(val):
    if val>np.median(train_data['duration']):
        return 'yes'
    return 'no'
train_data['duration']=train_data['duration'].map(dur_cat)

def camp_cat(val):
    if val>np.median(train_data['campaign']):
        return 'yes'
    return 'no'
train_data['campaign']=train_data['campaign'].map(camp_cat)

def pdays_cat(val):
    if val>np.median(train_data['pdays']):
        return 'yes'
    return 'no'
train_data['pdays']=train_data['pdays'].map(pdays_cat)

def prev_cat(val):
    if val>np.median(train_data['previous']):
        return 'yes'
    return 'no'
train_data['previous']=train_data['previous'].map(prev_cat)


attribute_values = {'age':['yes','no'],
                    'job':['admin', 'unknown', 'unemployed', 'management','housemaid','entrepreneur','student','blue-collar','self-employed','retired','technician','services'], 'marital':['married', 'divorced', 'single'], 'education':['unknown', 'secondary', 'primary', 'tertiary'], 
                    'default':['yes','no'],
                    'balance':['yes','no'],
                    'housing':['yes', 'no'],
                    'loan':['yes', 'no'],
...
#maximum tree depth - 1 to 16 - user input
...
class ID3:
    ...
    def exp_entropy(self, data, attributes):
        ...
        for value in attribute_values[attributes]:
            value_label = data[data[attributes] == value]['y']
